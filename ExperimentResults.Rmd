...
df <- rename(df, pfClaimScore=Answer.personalFinanceClaimScore, overclaimScore=Answer.overclaimingScore, flQuizScore=Answer.FLQuizScore, accuracy=Answer.accuracy, pfFirst=Answer.personalFinanceFirst)
plot(df$pfClaimScore, df$overclaimScore)
```
...
```{r}
data.lm = lm(formula = df$overclaimScore ~ df$pfClaimScore + df$accuracy, data = df)
...
```{r}
order.lm = lm(formula = df$overclaimScore ~ df$pfFirst, data = df)
kable(summary(order.lm)$coefficients)
```

Get slope, t value and p for data when personal finance questions are asked first only.
```{r}
PFFirst.lm = lm(formula = personalFinanceFirstData$overclaimScore ~ personalFinanceFirstData$pfClaimScore + personalFinanceFirstData$accuracy, data = personalFinanceFirstData)
kable(summary(PFFirst.lm)$coefficients)
```

Get slope, t value and p for data when 15 item questions are asked first only.
```{r}
itemsFirst.lm = lm(formula = itemQuestionsFirstData$overclaimScore ~ itemQuestionsFirstData$pfClaimScore + itemQuestionsFirstData$accuracy, data = personalFinanceFirstData)
...
```{r}
genuineKnowledge.lm = lm(formula = df$overclaimScore ~ df$flQuizScore, data = df)
...
```{r}
perceivedKnowledge.lm = lm(formula = df$pfClaimScore ~ df$flQuizScore, data = df)
